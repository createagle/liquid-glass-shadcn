# Liquid Glass UI —— 交付给 Claude 的项目提示词

> **怎么用这份文件**
>
> 1. 把「第一部分：主提示词」整段存为仓库根目录的 `PROJECT_SPEC.md`，并在 `CLAUDE.md` 里写一行：`阅读并严格遵守 ./PROJECT_SPEC.md。任何实现决策与之冲突时，以 PROJECT_SPEC.md 为准。`
> 2. 「第二部分：阶段任务卡」按顺序一张一张粘给 Claude。**不要一次性把 Phase 0–7 全部丢过去** —— 50+ 组件的体量一次性推进必然跑偏。
> 3. 每个 Phase 结束后，让 Claude 逐条对照该 Phase 的「验收 checklist」自查并输出结论，再进入下一个 Phase。
> 4. Phase 1（光学引擎）是整个项目的地基，**不要为了赶进度跳过它的视觉比对环节**。

---
---

# 第一部分：主提示词（PROJECT_SPEC.md）
...
> 一次粘一张。每张结束后要求 Claude 逐条对照 checklist 自查并给出结论。

---

## Phase 0 · 研究与基线（不产出组件代码）

```
执行 PROJECT_SPEC.md 的第 1 节「强制第一阶段：研究」。

具体做三件事：

1. 抓取并精读第 1.1 节列出的全部 Apple 文档（记得用 .md 后缀绕过 JS 渲染），
   以及 1.3 的 Web 实现参考、1.4 的 shadcn registry 文档。

2. 产出 docs/research/ 下的四份笔记：
   - apple-liquid-glass.md（每条结论标注来源 URL）
   - apple-metrics.md（每个数值标注 [官方]/[实测]/[推定]）
   - optics-web.md（三级降级方案 + 各浏览器实测）
   - shadcn-registry.md（registry.json 与 registry-item.json 的完整字段说明、
     build 命令实测结果、namespaced 安装流程）

3. 从 https://ui.shadcn.com/docs/components 抓取当前实际的组件全量清单，
   写进 docs/research/component-inventory.md，
   按 PROJECT_SPEC 第 10 节的 P0/P1/P2/P3 分层，并逐个标注对应的 Apple 控件
   （没有对应物的明确写「无 Apple 对应，按内容层处理」）。

不要写任何组件代码。完成后向我汇报：
(a) 你从 Apple 文档中读到的、PROJECT_SPEC 里没提到的重要约束；
(b) 你认为 PROJECT_SPEC 中有哪些技术判断是错的或有风险的（直说，不要附和）；
(c) shadcn 当前组件总数，以及你的分层结果。
```

**验收**：四份笔记 + 清单存在；`apple-metrics.md` 里的数值都带可信度标注；(b) 给出了实质性的质疑而不是客套话。

---

## Phase 1 · 光学引擎 `@glass/core`（项目地基）

```
按 PROJECT_SPEC 第 5 节实现 packages/glass-core。不要写任何 UI 组件。

必须产出：
1. 位移贴图生成器 + 滤镜工厂（含尺寸缓存、全局单例 SVG defs 容器、ResizeObserver）
2. 三级 tier 检测（特性检测优先，UA 兜底），结果写到 <html data-glass-tier>
3. <GlassProvider>：主题（light/dark）、材质档位（0..1 连续）、tier 覆写、
   无障碍偏好订阅（reduced-transparency / reduced-motion / contrast）
4. <GlassSurface> 底层原语，接受 layer="base" | "indicator" | "elevated"
5. springs 预设导出（smooth / snappy / bouncy）
6. 一个纯 HTML 的调试页（不依赖 Next.js）：
   把 <GlassSurface layer="indicator"> 叠在一张高对比度的彩色照片 + 一段小字号文本上，
   暴露所有滤镜参数为可拖拽的滑杆。

然后做视觉标定（这一步最关键，不要跳）：
- 用我提供的 iOS 截图（或你自己截的）作为基准
- 反复调参，直到「指示器」的三个特征都肉眼可见：
  边缘透镜畸变、可见的彩虹色散边、上缘镜面高光
- 把最终参数固化为 --lg-refract-* / --lg-disperse-* 的默认值
- 在 docs/research/optics-web.md 里贴出调试页截图和最终参数
```

**验收**：调试页在 Chrome 下能看到明确的色散彩边和背景畸变；Safari（Tier B）下看起来仍然是完整设计；Firefox（Tier C）下结构与可读性完全正确；三个 tier 可手动强制切换。

---

## Phase 2 · Token 体系与明暗模式

```
按 PROJECT_SPEC 第 6、7、8 节实现完整 token 层。

1. 三层 token 全量定义（Tailwind v4 @theme + CSS 变量），
   light / dark 各自独立完整定义一套
2. Layer 3 必须覆盖 shadcn 的全部 token 名（去 shadcn 官网核对，别凭记忆）
3. 材质档位：0..1 连续插值，映射 4 个语义档位，只影响 Layer B
4. localStorage 持久化 + SSR 内联脚本防闪烁（主题和档位共用一套机制）
5. squircle 连续圆角方案 + concentricRadius() 工具函数
6. 输出一张 token 速查页（可以先是静态 HTML），
   展示全部色板、材质在 4 个档位 × 明暗 2 种下的 8 种表现

自查：随便挑 3 个第三方 shadcn 组件（比如官方的 button、card、dialog）
装进这套 token 下，确认样式不崩。
```

**验收**：8 宫格材质表现图正确；第三方 shadcn 组件在本 token 下正常渲染；无暗色闪烁。

---

## Phase 3 · P0 组件（11 个，质量优先于速度）

```
按 PROJECT_SPEC 第 10 节的 P0 清单实现 11 个组件。

顺序建议：Segmented/Tabs → Slider → Switch → Button → Sheet/Drawer →
Dialog/AlertDialog → Select → DropdownMenu → Popover → Toggle → Card

先做 Segmented/Tabs，因为它是「Layer B 磨砂底座 + Layer I 强玻璃指示器」
这个核心分层最纯粹的样本 —— 它做对了，剩下的都好办。

每个组件必须：
- 顶部写 // APPLE REFERENCE: 注释块
- 走完 PROJECT_SPEC 第 14 节的验收 checklist 全部条目
- 附一张 iOS 截图 vs 组件截图的并排对照图，存到 apps/www/public/fidelity/

一次只交付 2–3 个组件，让我看过再继续。
不要为了凑数量牺牲还原度 —— 这 11 个是整个库的门面。
```

**验收**：逐个对照 checklist 全绿；Fidelity 对照图存在且差异被诚实描述（不要说"完全一致"）。

---

## Phase 4 · ResponsiveOverlay 与移动端适配

```
按 PROJECT_SPEC 第 9 节实现 <ResponsiveOverlay> 原语，
并把 Phase 3 中所有下拉类组件（Select / DropdownMenu / Popover）迁移过去。

重点（这几条最容易做错）：
- 用 useSyncExternalStore 订阅 matchMedia，杜绝 SSR hydration mismatch
- Drawer 的 snap points / detents、拖拽把手、速度感应甩动关闭
- 背后页面的缩放后退效果（iOS 层叠观感）
- safe-area-inset-bottom 适配
- 焦点陷阱与焦点还原在两条渲染路径下都要过测试

用 Playwright 在移动 viewport 下写 e2e：
打开 → 拖拽 → 甩动关闭 → 焦点还原，全流程断言。
```

**验收**：真机或移动模拟器下交互手感接近 iOS；无 hydration 警告；e2e 全绿；桌面路径行为未受影响。

---

## Phase 5 · Registry 分发打通

```
按 PROJECT_SPEC 第 11 节，把已完成的组件全部接入 shadcn registry。

1. 写 registry.json（组件多就用 include 拆文件）
2. 每个 item 配好 files / dependencies / registryDependencies / cssVars / css
   —— 特别是 cssVars，要保证组件装到别人项目里能自动带上所需的玻璃 token
3. 单独做一个 registry:theme item，让用户可以只装主题
4. 跑通 build，产物落在 apps/www/public/r/
5. 在一个全新的干净 Next.js 工程里，实测两种安装方式：
   - 直链：shadcn add https://.../r/button.json
   - 命名空间：components.json 配 @glass，然后 shadcn add @glass/button
6. 把这个冒烟测试写成 CI job，每次 PR 自动跑

汇报时贴出干净工程里安装后的实际运行截图。
```

**验收**：两种安装方式都实测通过；CI job 存在并通过；安装后组件在对方工程里样式完全正确（不需要手动补 CSS）。
...
```
按 PROJECT_SPEC 第 12 节构建 apps/www 文档站，结构对齐 ui.shadcn.com。

优先级顺序：
1. 组件页模板（Preview/Code 切换 + 安装命令 tabs + Examples + API 表）
   —— 先把模板打磨好，再批量填内容
2. API 表从 TS 类型自动生成，不要手写
3. 全局顶栏：明暗切换 + 材质档位滑杆 + tier 切换
4. Fidelity 标签页（本库独有卖点）
5. 首页 Hero：一个完全可交互的 iOS 风格界面
6. ⌘K 命令面板、代码复制、/view/[name] 独立预览路由
7. Docs 章节：Materials（讲 Layer B vs Layer I）和 Optics（讲三级降级）
   要写透 —— 这两页是本库和其他"毛玻璃 UI 库"的分水岭

文档站的导航、侧边栏、搜索面板、代码块工具栏必须用本库组件搭建。
```

**验收**：站点可用；顶栏三个全局控件生效并影响全站；Materials / Optics 两页内容扎实且配图；文档站自身确实在用本库组件。

---

## Phase 7 · P1–P3 组件批量推进

```
按 P1 → P2 → P3 的顺序继续实现剩余组件。

节奏要求：
- 每批 4–6 个组件，一批一交付
- 每个组件仍然要走完整的验收 checklist —— 不因为是 P2/P3 就降低标准
- 内容型组件（Table / List / Accordion / Card 正文）严格遵守
  「材质属于控件层」原则，不要堆玻璃
- 每批完成后补齐：registry item、文档页、视觉回归快照

每批交付时汇报：这批里哪个组件最难还原、你做了什么妥协、哪些地方
你认为还没达到 Apple 的水准。
```

**验收**：每批 checklist 全绿；组件覆盖率对齐 Phase 0 的 component-inventory；CI（安装冒烟 + 视觉回归 + 对比度检查）全绿。

---

## 通用收尾任务卡（每个 Phase 结束时都可以用）

```
逐条对照本 Phase 的验收 checklist 和 PROJECT_SPEC 第 14 节，自查并输出结论。

要求：
- 每条给出「通过 / 部分通过 / 未通过」，未通过的说清楚差在哪
- 明确列出你做了但 PROJECT_SPEC 没要求的事，以及理由
- 明确列出 PROJECT_SPEC 要求了但你没做到的事 —— 不要含糊带过
- 指出你在实现过程中发现的、PROJECT_SPEC 里判断有误或不切实际的地方

不要给我"全部完成 ✅"这种结论。我需要知道真实的缺口在哪。
```
...
| 取舍点 | 本提示词的选择 | 备选 |
|---|---|---|
| 光学引擎分发 | 独立 npm 包 `@glass/core` | 也塞进 registry（用户可改，但升级困难） |
| 组件分发 | shadcn registry（源码落地） | 发成 npm 组件包（省事但失去可定制性） |
| 材质档位 | 连续值 + 4 个语义档位 | 纯 4 档离散（实现简单，但少了 iOS 的顺滑感） |
| 档位影响范围 | 只影响 Layer B 底座 | 也影响指示器（更贴近 iOS，但通透档下玻璃感会丢） |
| 移动端判定 | 视口宽度 **或** 粗指针 | 只看视口宽度（平板会误判） |
| squircle | 大圆角才用，小圆角回退 | 全量 squircle（性能开销大） |
