...
const GEOMETRY = {
  /** 面板宽。[实测] 300pt —— 这是 **Alert** 的宽度，不是通用弹窗的宽度。 */
  width: 300,
  /** 圆角。[实测] —— 由参考图边缘轮廓拟合：34 个采样点，均方误差 0.35 */
  radius: 34,
  /** 面板内边距（四周）。[实测] */
  padding: 14,
  /** 正文块相对面板内边距再内缩。[实测] */
  ...
  /** 正文块 → 按钮区。[实测] */
  sectionGap: 24,
  /** 按钮之间。[实测] —— 132 + 8 + 132 = 272 = 300 − 28，自洽 */
  buttonGap: 8,
  /** 标题与正文的行高。[实测] —— 由两行正文的基线间距（27−4）读出 */
  lineHeight: 22,
  /** 标题与正文的字号。[实测] —— 两者墨迹高度都是 13px，即同一个字号 */
  fontSize: 17,
} as const;
