...
export const MENU_GEOMETRY = {
   ...
   * 面板圆角。[实测] **34**。
   *
   * ✅ 2026-09-04 更正：原值是推定的 22（当年渲染图拟合不收敛，退而取
   * `--lg-radius-lg`）。这次直接读节点属性 —— 就是当年量这个菜单用的
   * 同一个节点 `12740:24185`，它内部的 `Fill + Shadow` / `Glass Effect`
   * 两层都写着 `cornerRadius: 34`。
   *
   * 独立印证：iOS Context Menu（`125:58750`）的面板同样是 34，
   * 且菜单项同样是 218×40 —— 两个互不相关的节点给出同一块面板。
   * 记录见 apple-metrics.md §12.2 / §12.3。
   */
  radius: 34,
  /** 菜单项高。[实测] */
  itemHeight: 40,
