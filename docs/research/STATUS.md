...
**当前阶段：Phase 5（Registry 分发）已完成 · 已修可读性地板，14 个对比度测点中 12 个达标**
...
- 🔴 **剩余 2 个测点仍未达 AA**：`base/tint-blue` 1.50、`base/tint-red` 1.51。
  根因不同 —— 系统色是**固定值**，既压不亮也压不暗，材质地板对它无效。
  Apple 自己的指引是「把颜色加在**背景**上，不要加在文字/符号上」
  （`apple-liquid-glass.md` §5 第 3 条），但本库仍需给着色标签一个答案。**方案未定。**
- 🔴 **`adaptive` 模式只有数学，没有探针。** `resolveLegibleAlpha` 已支持传入实测
  背景亮度范围并据此降低地板（实测暗色主题压暗背景可保持 0.22 全通透），
  但 `GlassSurface` **还没有实现逐元素探测**，目前传的恒为 `null`，
  行为等同 `guaranteed`。**这是「元素级」三个字目前唯一没兑现的部分。**
