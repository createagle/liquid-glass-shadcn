...
Phase 7 已交付两批（表单四件 + 小件五个），**P1 14 个里做完 9 个，还剩 5 个**。**
...
按阶段纪律，下一步继续 **Phase 7**。P1 剩下的 5 个：

- **Tooltip / Toast** —— overlay 类。Tooltip 有 HIG 的一句原文（macOS），
  Toast 没有直接对应物。两个都要新的 overlay 机制（Tooltip 需要
  `@radix-ui/react-tooltip`，Toast 需要 provider + 队列 + 计时器 + 滑动关闭）。
- **Input Group** —— 输入框 + 附件视图，依赖已经做完的 Input。
- **Checkbox / Radio Group** —— inventory 标着「无 iOS 对应」（是 macOS 控件）。
  做之前得先说清楚参考从哪来，否则整批都是 `[推定]`。

建议下一批走 **Tooltip + Toast + Input Group**（三个，但 Toast 一个顶两个）。
Checkbox / Radio 单独一批，因为它们要先解决参考来源问题 ——
**在没有 macOS 参考的前提下硬做，出来的东西每个数字都是推定的**，
那种组件进库之前应该先问一句值不值得。
