...
**P2 还剩 2 个。** `Data Table` 与 `Command` —— 这是 **P2 的收尾批**。

- `Data Table` 复用已完成的 `Table`（几何来自 macOS `NSTableView`，§11.4），
  加的是排序 / 选择 / 分页的**行为**，不是新材质。
- `Command` 两份资源都没有，而且理由明确：**Spotlight 是系统级的，App 画不出来**。
  它会和 Breadcrumb / NavigationMenu 同一档 —— 全部 `[推定]`，逐条写明借自哪里。
  搜索框那一部分可以借 macOS `Search Fields`（`480:760`，做上一批时顺带查到，几何可量）。

做完之后 P2 十六个全齐，**可以整体回头对一次 PROJECT_SPEC 第 14 节**。

🔴 **在那之前，有一条跨全库的 token 改动应该单独占一个提交**：`--lg-blue`
现在仍是上一代的 `#007AFF`，而 `#0088ff` 已经有**三份互相独立的实测**指向它
（iOS 控件 §7/§9、macOS 控件 §10、iOS 日期格 §14）。
按 `e325168`（三个旧推定圆角）的先例处理 —— 它会动约 270 张视觉快照
与对比度基线的棘轮，塞进组件批次没法审。详见 §0.78 第三节。
