...
| DropdownMenu | 12 项过 **11** |
| Select | 12 项过 **11** |
| **ResponsiveOverlay**（Phase 4 原语） | 任务卡四条重点全部落地并有测试 |
| **文档站**（Phase 6 两批） | 任务卡 7 项里的 1 · 2 · 3 · 4 · 6 |

> 「文档页」那一项这一批**真的过了**：Preview/Code、安装命令、自动生成的 API 表、
> **每个组件 2 个示例**、**Fidelity 对照**（8 张图 + 差异说明；没有图的 4 个逐条说明原因）。
> 每一条都有测试钉住，不是自我评估。
>
> 仍然扣分的项与文档站无关（材质档位只录默认档、只有 win32 视觉基线、
> Sheet 抓手的可见色散不达标等），逐条见各组件自己的自查。

### Phase 6 的下一批：**Materials 与 Optics**

任务卡：「Docs 章节：Materials（讲 Layer B vs Layer I）和 Optics（讲三级降级）
**要写透** —— 这两页是本库和其他『毛玻璃 UI 库』的分水岭」。

材料其实都在仓库里了，缺的是把它们组织成两页能读的东西：

- **Materials**：α=0 陷阱（Button 15.46 → 1.92 的实测）、
  为什么底座绝不折射、挖洞的四种写法实测对照（σ 0.5 / 127.5 / 89 / 圆角）、
  可读性地板的推导 `C = a·F + (1−a)·B`。
- **Optics**：三级降级各自是**完整设计**而不是残缺版、
  位移量为什么必须按短边比例而不是绝对像素（-180 在 85×54 上等于 ±90px 位移）、
  色散比例的 A/B 标定、以及**光学至今没有真机基准**这件事。

再往后：首页 Hero（任务卡第 5 位）、Themes / Playground、
Theming / Dark Mode / CLI / Registry 四页、代码块语法高亮。
...
- **补一条真正的 SSR 验证** —— 文档站本身已经是一个真实的 App Router 工程，
  `docs.yml` 每次 push 都构建它；缺的仍是「无 hydration mismatch」的显式断言。

### 这一段时间学到的三条

**一、快照回归有一块结构性盲区。** 1px 内描边在 250×220 的面板上只占 0.9%，
...
**三、`data-slot` 覆盖已经踩了四次。** SheetClose、ResponsiveOverlay、
DropdownMenu、命令面板，每次解法都一样（另起一个属性），
每次都是测试红了才发现。组件里的注释只有写组件的人读得到，
**调用方看不到** —— 该有一条 lint 规则。记着，没做。
...
> 文档站现在把这件事**摆到了明面上**：组件页的「尺寸常量与可信度」表里
> 橙色的「推定」徽章有多少个一眼能数出来；Fidelity 那一节的第一句话就是
> 「左边不是真机截图，可比的是几何、不是材质」。
