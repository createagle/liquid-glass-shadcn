...
Phase 7 第一批（表单四件）已交付，P1 还剩 10 个。**
...
按阶段纪律，下一步继续 **Phase 7**。P1 剩下的 10 个：

- **Checkbox / Radio Group** —— inventory 标着「无 iOS 对应」（是 macOS 控件）。
  做之前得先说清楚参考从哪来，否则整批都是 `[推定]`。
- **Input Group / Progress / Tooltip / Toast** —— 有 Apple 对应物，可以先找参考图。
- **Badge / Avatar / Separator / Skeleton** —— 内容层，几何简单，但要守住「不堆玻璃」。

建议下一批走 **Progress + Tooltip + Toast + Badge**：前三个有 Apple 对应物，
Badge 顺带把「小尺寸玻璃看不出效果」这条写进文档。
