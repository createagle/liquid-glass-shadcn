...
1. 🔴 **`--lg-blue` 从 `#007AFF` 改成 `#0088ff`** —— 四份独立实测了。
   按 `e325168` 的先例单独占一个提交（会动约 286 张快照 + 对比度基线的棘轮）。
...
1. 🔴 **`--lg-blue` 从 `#007AFF` 改成 `#0088ff`** —— 现在已有**四份**互相独立的实测
   （iOS 控件 §7/§9、macOS 控件 §10、iOS 日期格 §14、macOS 搜索框 §15.2）。
   按 `e325168`（三个旧推定圆角）的先例单独占一个提交 —— 它会动约 286 张视觉快照
   与对比度基线的棘轮，塞进组件批次没法审。
2. 🔴 **发布 `@glass/core` 到 npm** —— 否则真实用户装不了 registry item，
   冒烟测试至今靠本地 shim 顶着。这条从 Phase 5 挂到现在。
