...
## 0.78 P2 第四批 · Calendar / DatePicker / Combobox —— 自查（2026-09-04）

**这一批最值钱的一条不是几何，是一次 Apple 自证。**

PROJECT_SPEC §2 说「内容型组件不堆玻璃，材质属于控件层」。
这条规则此前一直只能**靠推理**落实 —— Card / Table / Accordion 不上玻璃，
理由是「它们是内容」，但 Apple 从没直说过。

这次资源里同一个日历有两个变体，**材质不一样**：

| | 圆角 | 左右内边距 | 材质 |
|---|---|---|---|
| `Style=Inline`（嵌在内容里） | 无 | 16 | **纯白不透明** `#ffffff` a=1 |
| `Style=Compact`（弹出层） | **13** | 12 | **玻璃**（Material，`GLASS r=16`，落影 r=48） |

> **同一个控件，嵌进内容就是白的，浮成弹层才是玻璃。**
> 这是本库第一次拿到这条规则的**直接证据**，而不是推论。
> 组件因此分成两个：`<Calendar>` 内容层、`<DatePicker>` 弹层 Layer B ——
> 分法来自资源，不是来自我们对 SPEC 的解读。

### 一、面板圆角 13 —— 复核过两遍，是真的

菜单 34、Popover 38、这里 **13**。第一眼以为量错了，回去把 Compact 变体的
`Fill + Shadow` 与 `Glass Effect` 两层都读了一遍，都是 13。

顺手给 `PopoverContent` 加了 `radius` / `paddingBlock` / `paddingInline` 三个 prop。
理由写在 prop 的注释里：**不同浮层的圆角是不一样的，不该当成一个全局常量** ——
而调用方用 className 是盖不住的（`GlassSurface` 的 radius 是内联样式）。

### 二、两条反直觉的实测，都钉了测试

- **日历选中态是黑底白字，不是主题蓝。** 实测 Selected = 实心 `#000000` + 白字
  Semibold；只有「今天且被选中」才是实心 `#0088ff`。多数 Web 日历把选中画成
  主题色 —— 这条测试钉的就是「别跟」。
- **DatePicker 触发器打开时只有文字变蓝，底色不变。** 与本库大多数控件
  「选中改底色」的直觉相反，资源里就是这样。

### 三、🔴 顺手撞出一条**跨组件的 token 问题**（本批没改）

写「今天那一格是蓝字」的测试时，断言 `#0088ff` 失败了 ——
量到的是 **`rgb(0, 122, 255)`**，也就是 `--lg-blue` 仍然是上一代的 **`#007AFF`**。

而 `#0088ff` 现在已经有**三份互相独立的实测**指向它：

| 来源 | 值 |
|---|---|
| iOS 27 控件 / 菜单（§7、§9） | `rgb(0 136 255)` |
| macOS 27 控件强调色（§10） | `#0088ff`（亮）/ `#0091ff`（暗） |
| iOS 27 日期格「今天」与折叠字段选中态（§14.2 / §14.5） | `rgb(0 136 255)` |

§15 的未决问题 #4（「`rgb(0 136 255)` vs `#007AFF` 是色彩管理还是真改值」）
**其实早在 §10 就答完了** —— 两份独立资源逐位相同，不可能是某一份的色彩管理误差。
但 token 一直没跟着改。

🔴 **本批刻意没有改它。** 理由：那是一次跨全库的改动（每个用到蓝的组件、
约 270 张视觉快照、对比度基线的棘轮都会动），塞进组件批次会让这个提交没法审。
按 `e325168`（三个旧推定圆角）的先例，**它应该单独占一个提交**。

> 测试里因此断言的是「这一格用了 `--lg-blue` 这个 token」，**不是**某个字面色值 ——
> 这样 token 改了之后测试不用跟着改，而且现在就不会把错的值钉死。

### 四、自己写日历网格，代价与收益

没有引 `react-day-picker`。理由两条（都写进了组件头部）：几何是全实测的，
套在别人的 DOM 上够不到就得写死魔法数字；而且它自带一套 class 命名与
`components` 覆写协议，与本库的 `data-slot` 约定是两套东西。

代价是键盘导航、月份切换、闰年边界全是本库自己的。测试因此逐条钉住：
方向键走格 / Home-End 走一周 / **方向键跨月自动翻页** / roving tabindex 只有一个
tab stop / **月份按钮在「1 月 31 日 + 1 月」上不溢出**（这条是真踩过的：
直接 `setMonth` 会跑到 3 月 3 日）。

### 五、踩的坑，四个

- **`rounded-full` 在 Tailwind v4 里是 `calc(infinity * 1px)`**，计算值 33554400。
  视觉上没问题，但「正圆 = 边长的一半」这条断言就没法写了 ——
  而本库的规矩是几何必须能从常量核对回去。改成用常量算出来的 19。
- **harness 里传了 `month` 就把日历变成受控的**，月份按钮点了没反应，
  两条键盘测试直接超时 5 秒。第一版就是这么写的。
- **`CSS.escape` 是浏览器 API**，写在 Node 侧的测试文件里是 `ReferenceError`。
  改用属性选择器。
- **视觉快照里选中格是灰的** —— 状态示例把周末设成不可选，而选中日 4/12 正好是周日，
  选中 + 禁用叠在一起被 `opacity: 0.3` 压灰了，「黑底白字」那条根本看不出来。
  换成 4/15（周三）。**示例内容本身也会让快照失去意义**，这是新的一条。

### 六、又双叒撞上既有 token（第四次）

| 实测 | 命中的 token |
|---|---|
| 折叠字段两枚胶囊 `#767680 @ 0.12` | `--lg-fill-tertiary`（R/G 各差 2） |
| 星期缩写 `#3c3c43 @ 0.30` | `--lg-label-tertiary`（**逐位相同**） |

本批同样**没有新增任何颜色 token**。

### 七、欠着的，如实列

- **不做时间选择。** 资源里时间那一枚点开是滚轮（UIDatePicker 的 wheels 形态），
  那是另一个控件、另一套几何，**本批没有量**。`time` 那一枚是只读展示。
  「没做」与「没有依据」是两回事 —— 这里是前者。
- **Combobox 的默认过滤只是大小写不敏感的子串匹配** —— 没有拼音、没有模糊、
  没有权重排序。那些是 `Command` 的活，而 `Command` 还没做。
- **Combobox 的尺度是借的。** 结构与三档按钮底色是 macOS 实测，
  高 44 / 字号 17 / 内边距全部 `[推定]`（借本库的 Input）。组件里逐条标了。
- **`--lg-blue` 那条**，见上面第三节。
...
| 行为测试 | **401 通过**（+26） |
| 视觉快照 | **272 通过**（+14） |
| 文档站测试 | **45 通过**，三个新页都进了控制台零告警名单 |
| 对比度审计 | 1512 采样全达标 |
| registry-lint | 122 个文件通过 |
| `shadcn registry validate` | 42 项 / 2 文件，有效 |
| typecheck | 干净 |
| 尺寸常量 | **296 个，全部带可信度标注，0 个没有** |

组件 39 → **42**，示例 74 → **80**。

> ⚠️ 生成脚本的可信度闸门这次**真的拦住了 4 个**（`CALENDAR_GEOMETRY.columns`
> 与 `BUTTON_FILL` 的三档）。补标签才过。那道 `process.exit(1)` 不是摆设。

### 九、P2 还剩 2 个

`Data Table` 与 `Command`。

- **`Data Table`** 复用已完成的 `Table`（几何来自 macOS `NSTableView`，§11.4），
  加的是排序 / 选择 / 分页的**行为**，不是新材质。
- **`Command`** 两份资源都没有，而且**有明确理由**：Spotlight 是系统级的，
  App 画不出来。它会和 Breadcrumb / NavigationMenu 同一档 —— 全部 `[推定]`，
  逐条写明借自哪里。搜索框那一部分可以借 macOS `Search Fields`（`480:760`，
  已顺带查到，几何可量）与本批 Combobox 的列表。

**这一批是 P2 的收尾批。** 做完之后 P2 十六个全齐，可以整体回头对一次
PROJECT_SPEC 第 14 节。

## 0.79 P2 收尾批 · DataTable / Command —— 自查（2026-09-05）

**P2 十六个到此全齐。** 这一批本身很小，但顺手挖出了一个**挂了两个月的洞**，
那件事比两个组件都重要 —— 见第三节。

### 一、两条结论，一半是「不用量」

第五次「先量再定」。这次的答案有一半是**否定式**的：

- **DataTable 不需要任何新几何。** 回去把 macOS `Lists and Tables` 又翻一遍：
  列表头、整条表头 600×28、排序指示器（Medium 9 / 13×19 / `#000000 @ 0.50`）、
  数据行与交替行与两档选中 —— **全都已经在 §11.4 量过并实现在 `<Table>` 里**。
  增量是**排序状态机 / 行选择 / 分页**，那是**交互**，不是外观。
  所以这个组件里**没有 GEOMETRY 常量块**，一个数字都没有新增。

  > 「量过了，发现没有新的可量」与「懒得量」是两回事。
  > 前者要写下来，否则下一个人还得再翻一遍。

- **Command 在 Apple 资源里结构上不可能有。** 它对应的是 **Spotlight** ——
  那是**系统级**界面，App 画不出来，所以设计资源里不会有它的样例。
  这与 Toast 的「系统通知横幅」是同一类：不是「没找到」，是**不会有**。

  唯一可量的一半是**搜索框**（macOS `480:760`，6 个状态）：胶囊、放大镜在前
  `#000000 @ 0.85`、清除在后 `@ 0.5`、光标 2×18 `#0088ff`。
  ⚠️ 尺度照旧不取（24 高 / 13 号字是鼠标语境），只取结构与配色。

  > 顺带：这里的光标又是 `#0088ff` —— 那是该值的**第四份**独立来源。
  > `--lg-blue` 仍是上一代的 `#007AFF`，见 §0.78 第三节，仍然欠着。

### 二、Command 的分层也要改：不是「B + I」

清单第 39 行写的是 `B + I（高亮项）`。高亮项是一层**平涂填充**，没有折射；
而且命令面板一屏十几项，每项上折射会撞穿 §5.2 的 8 实例红线 ——
与 §0.78 给 Calendar 改的理由完全一样。这是这一族第三次改（Menubar / Calendar / Command）。

### 三、🔴 最重要的一件：**lint 脚本的注释剥离器在吞真实代码**

写「给组件传 `data-slot` 会被静默吃掉」那条新规则时，发现它在探针文件上
**一次都不报**。查下去查到了根：

```js
// 原来的第一条 strip 规则
.replace(/\{\s*\/\*[\s\S]*?\*\/\s*\}/g, '')   // 本意：删 JSX 注释 {/* … */}
```

中间虽然非贪婪，但**尾部的 `\s*\}` 会逼它继续往后找** ——
一个普通块注释（`*/` 后面不是 `}`）会让它一路吞到**下一个** `*/}`，
把中间的代码整段删掉。`calendar.tsx` 里的 `function Calendar(` 就是这么消失的。

**后果不是报错，是规则悄悄少查了一大片文件。** 已有的 `no-as-child` 也一样受影响 ——
也就是说那条从 2026-09-01 就在的规则，实际覆盖面一直比看上去小。

> **一条永远查不到东西的 lint 规则，比没有规则更糟** ——
> 它会让人以为这类问题已经被守住了。
>
> 这一批里同一个形态出现了**三次**：
> ① strip 规则吞代码；② 新规则的正则被改写脚本吃掉一层转义，
> 变成「退格符 + data-slot」，语法合法、跑得过、永不报错；
> ③ owner 检测里 `indexOf('{')` 找到的是**解构参数**那个花括号，
> 「函数体」只有参数列表那么长，什么都匹配不到。
>
> **三次都是「跑得过 + 什么都没查」。** 所以最后加了一步：
> 放一个**故意违规的探针文件**进去，确认规则**真的会红**，再删掉。
> 以后写 lint 规则都该这么验一次。

### 四、新规则的范围被误报逼着收窄了两次

- 第一版查「所有大写开头的 JSX 标签」→ 把 `<AccordionPrimitive.Content data-slot=…>`
  这类**正当写法**全报了（Radix 原语会把不认识的属性原样透传到 DOM）。
- 第二版收窄到「从 `@/components/ui/*` 引进来的名字」→ 还是误报：
  `<PopoverAnchor>` / `<ResponsiveOverlayTrigger>` **自己不写 data-slot**，
  透传下去完全正常。

最终判据是**目标组件自己写不写 `data-slot`**（即函数体里 `{...props}` 之后
是否还有 `data-slot=`），按 registry 全量源码建索引再判。

**规则一上线就抓出三处真 bug**，其中一处是**上一批留下的、没有任何测试发现的**：
`date-picker.tsx` 里 `<Calendar data-slot="date-picker-calendar">` ——
属性从来没有出现在 DOM 上。

### 五、DataTable 的两处刻意偏离

- **排序是三态循环**（升 → 降 → **回到原始顺序**）。macOS 的表头是两态。
  理由：一旦排过就再也回不到原始顺序，而原始顺序往往本身有意义
  （后端可能已经按相关度排好了）。
- **分页复用 `<Pagination>`（iOS Page Control，圆点）**，不是「上一页 / 下一页 / 页码」
  那种 Web 分页条 —— 那种形态两份资源里都没有，本库不自造。

**不引 TanStack Table**：registry 分发的是源码，多一个依赖就是用户工程里多一个；
而这里只用到排序 / 选择 / 分页三件事。代价是**没有**列可见性切换、列宽拖拽、
分组、虚拟滚动、多列排序 —— 需要那些就直接用 TanStack + 本库的 `<Table>` 原语。

### 六、一条写砸了又改回来的测试

`空数据时给出提示` 第一版的断言是「查一下 `[data-slot=data-table]` 在不在」——
**名字说的是空态，断言的却是「组件渲染了」，根本没走到那条分支**。
harness 里补了一个 `data={[]}` 的实例，现在是真的在测空态（含 colSpan 铺满整行）。

> 与第三节是同一类问题的不同外壳：**「绿」不等于「查到了」。**

### 七、一处已知的间歇性失败（不是本批引入）

`sheet.behavior.spec.ts › 甩动关闭` 在一次全量并行跑里红过一次，
单独跑 23/23 通过，之后全量连跑两轮 421/421 全绿。
它是**基于速度**的手势断言，并行负载下指针事件的时间戳会被拉长。

🔴 **没有修，如实记着。** 按本仓库的规矩，「重跑一遍看看」是错的反应 ——
这里先确认了产物是新的、且能稳定复现绿，再把它登记成已知 flake。
真要修，得让手势测试用**注入的时间戳**而不是真实时钟。

### 八、验证

| 项 | 结果 |
|---|---|
| 行为测试 | **421 通过**（+20；连跑两轮全绿） |
| 视觉快照 | **286 通过**（+14） |
| 文档站测试 | **47 通过**，两个新页都进了控制台零告警名单 |
| 对比度审计 | 1512 采样全达标 |
| registry-lint | 128 个文件 · **2 条规则**（新规则已用探针验过真的会红） |
| `shadcn registry validate` | 44 项 / 2 文件，有效 |
| typecheck | 干净 |
| 尺寸常量 | **308 个，全部带可信度标注，0 个没有** |

组件 42 → **44**，示例 80 → **84**。

### 九、P2 十六个全齐 —— 下一步是回头对 SPEC

| # | 组件 | 状态 |
|---|---|---|
| 29–32 | Accordion / Collapsible / ScrollArea / Table | ✅ §0.74 |
| 34–35, 38, 44 | Pagination / Breadcrumb / ContextMenu / Resizable | ✅ §0.76 |
| 36–37, 43 | NavigationMenu / Menubar / Sidebar | ✅ §0.77 |
| 33, 39–42 | DataTable / Command / Combobox / Calendar / DatePicker | ✅ §0.78 / 本节 |

**下一步不是继续加组件，是回头对一次 PROJECT_SPEC 第 14 节**，
并把两件挂着的跨库改动排上：

1. ✅ **`--lg-blue` 从 `#007AFF` 改成 `#0088ff`** —— 已做，见 §0.81。
   （顺带说明：这里预测的「会动约 286 张快照 + 对比度基线的棘轮」**两条都错了**，
   一张快照没动、基线也没动。原因见 §0.81 第三节，那条比改色本身更值得记。）
...
## 0.81 `--lg-blue` 换成实测的 `#0088ff` —— 以及「视觉快照根本不管颜色」（2026-09-05）

按 `e325168`（三个旧推定圆角）的先例，单独占一个提交。

### 一、依据：四份互相独立的实测

| 来源 | 语境 | 值 |
|---|---|---|
| iOS 27 Slider 已填充轨道（§7.4） | 亮 | `rgb(0 136 255)` |
| macOS 27 Checkbox/Radio 选中底 + 焦点环（§10.3 / §10.4） | 亮 | `#0088ff` |
| iOS 27 日历「今天」、折叠字段选中态（§14.2 / §14.5） | 亮 | `rgb(0 136 255)` |
| macOS 27 搜索框文本光标（§15.2） | 亮 | `#0088ff` |

四份逐位相同，`#007AFF` 是上一代的值。§7.4 当初写的「差异方向与
Display P3 → sRGB 一致，本项不得据以修改 token」的顾虑，到此可以排除。

- `--lg-ios-blue-light`：`#007aff` → **`#0088ff`**
- `--lg-ios-blue-dark`：`#0a84ff` → **`#0d9eff`**

> ⚠️ **暗色只有一份来源，而且是 macOS。** §10.3 的暗色选中底是
> `#0091ff` + `#0d0d0d` LINEAR_DODGE，等效实色 `#0d9eff`；这里取**等效值**，
> 与亮色用的是同一条规则（「Idle 态渲染出来的强调填充」）。
> iOS 资源里一个暗色样例都没有。**亮四份、暗一份，不是同等确定**，
> 这句话同时写在 `primitive.css` 的注释里，免得日后被当成等价证据。

### 二、代价：白字更不占便宜

新蓝更亮，白字压上去对比度**更低**，`deriveProminentFill()` 的修正因此要拉得更狠：

| | 原色白字 | 解出的填充 | 每通道位移 |
|---|---|---|---|
| 亮 | 4.02 → **3.52** | `#0071eb` → **`#0075da`** | G/B −9/−20 → **−19/−37** |
| 暗 | 3.65 → **2.86** | `#0973df` → **`#0a79c4`** | G/B −17/−32 → **−37/−59** |

暗色那一档现在是**全库偏离 Apple 最远的一个色值**。写在 `semantic.css` 里，
免得下次有人以为是算错了。换黑字能一步解决（`#0d9eff` 黑字 7.35:1），
但 iOS 的蓝色 prominent 按钮就是白字 —— 极性属于设计语言，不由算法挑。

派生值全部由 `scripts/derived-colors.mjs --write` 重解，没有一处是手填的。

### 三、🔴 两条预言都错了，而且错得有价值

上一节（§0.79）写「它会动约 286 张视觉快照与对比度基线的棘轮」。**两条都没发生。**

**1. 对比度基线纹丝不动。** 1512 次采样全达标，一条「可收紧」的提示都没有。
原因：审计采的是**文字**颜色，而文字走的是 `--lg-on-glass-*`，
那一族被重解之后仍然精确落在 5.6:1 附近 —— 目标没变，起点变了，结果自然还在原处。

**2. 286 张视觉快照一张都没动。** 这条更值得记。

Playwright 的默认 `threshold: 0.2` 是 pixelmatch 的**逐像素 YIQ 距离**阈值
（0.2 × 35215 ≈ **7043**）。本次所有色变都在两个数量级之下：

| 变化 | delta | 阈值 |
|---|---|---|
| `--lg-blue` 暗（最大的一处） | **175** | 7043 |
| `--lg-blue` 亮 | 49 | 7043 |
| `accent-fill` 暗 / 亮 | 57 / 22 | 7043 |
| `on-glass-blue` 亮 / 暗 | 6 / 4 | 7043 |

> **那 286 张快照守的是几何与版式，不是颜色。**
> 这个阈值本来就是为抗锯齿噪声调的。之前一直默认「快照会兜住色差」，
> 是个没验证过的假设 —— 这次是**第一次**有明确反例。

**不打算调紧它。** 已实测 Windows 与 Linux 的 blur 渲染不一致（§0.5），
调紧会让整套快照在平台间全红。颜色归**行为测试**用 `getComputedStyle` 钉。

### 四、于是补了两条字面色值断言

此前日历「今天」那条测试**刻意**只断言「走了 `--lg-blue` 这个 token」，
理由写在当时的注释里：token 还没改，不想把错的字面值钉死。理由成立，
但它留下一个洞 —— **token 本身被改错，那条测试照样绿**。

现在 token 定了，补上另一半：

- `date.behavior`：`--lg-blue` 解析出来必须是 `rgb(0, 136, 255)`；
- `button.behavior`：实心填充 `!==` `--lg-blue`（**与 token 比，不抄字面值**），
  且 `--lg-blue` 是 `rgb(0, 136, 255)`、填充是 `rgb(0, 117, 218)`。

> 顺手修掉一条**已经变成空话的断言**：button 那条原来写的是
> `expect(bg).not.toBe('rgb(0, 122, 255)')` —— 抄的是上一代 systemBlue 的字面值。
> token 一改，它就变成「不等于一个本库根本不再使用的颜色」，永远成立。
> 改成与当场读出的 `--lg-blue` 比，token 再变它还成立。

### 五、顺手清掉的陈旧数字

改 token 逼出一批注释复核，其中一条**在此之前就已经是错的**：

`legibility.ts` 里「实测 `--lg-blue` / `--lg-red` 当标签色时只有 **1.50 / 1.51:1**」——
对不上任何一版的最不利底座（现在是亮 1.84 / 1.85、暗 2.35 / 1.97）。
那是更早一版底座下的数，**而没有任何东西会因为它过时而报错**。
换成 `derived-colors.mjs` 原样打印的那一列，下次谁动了 token 或底座，一跑就看得出来。

同类清理：`slider.tsx` / `progress.tsx` 里「不采信为 token」的颜色告警
（现在采信了，两个文件的代码一个字没动，值自己对上了）、
`button.tsx` 的 4.02 与「调整量很小」、`fidelity.html` 已知差异①、
以及 `button.tsx` 里 `plain` 变体注释引的 4.02 ——
那个数是**白字压在蓝底**上的，而 `plain` 是**蓝字压在玻璃**上，
该引的是 1.84，一直引错了。
...
|---|---|
| 行为测试 | **421 通过** |
| 视觉快照 | **286 通过**（一张未变，原因见第三节） |
| 对比度审计 | 1512 采样全达标，**基线未动** |
| `derived-colors` | 22 个派生 token 全部与推导一致 |
| registry-lint | 128 文件 · 2 规则 · 45 个 item 产物逐字核对 |
| `shadcn registry validate` | 45 项 / 2 文件，有效 |
| typecheck | 干净 |

### 七、没做的

- **PROJECT_SPEC §6 的系统色表没有改。** 按 Switch（§7.3，51×31 那条）的先例，
  SPEC 的修订登记在 `apple-metrics.md` 的「已确认的 PROJECT_SPEC 错误」表里，
  不直接改 SPEC。现在那张表有**两行**了。
  值得注意的是 SPEC 给整张色表标的是「已核实，直接用」——
  现在已知**至少一行是错的**，其余八色仍未逐个核对。
- **其余八个系统色一个都没动。** 只有蓝色有实测证据。
...
## 0.83 线上冒烟检查 —— 以及探针把检查本身查出一个 bug（2026-09-05）

§0.82 末尾记的「没做的」第二条：部署失败会红，但**页面渲染坏了不会**。补上。

### 一、只测本地测不出来的那部分

构建期已经有两道闸（`check-export-links.mjs` 扫绝对路径、
`registry-lint.mjs` 逐字核对 registry 产物），但它们看的都是**磁盘上的文件**。
「文件是对的」和「站点是活的」之间隔着托管层，而托管层有自己的坑：

| 只有线上能发现的 | 为什么本地看不出来 |
|---|---|
| `_next/` 取不到（`.nojekyll` 丢了） | Pages 默认过 Jekyll，会丢掉所有 `_` 开头的目录。**HTML 照样 200、页面照样返回**，只是没样式没脚本 |
| registry item 取不到 | §0.80 那 8 个组件就是「入库了但没发布」，本地一切正常 |
| 安装页登的地址是死的 | 此前登的 `https://liquid-glass-ui.dev/…` **域名根本不存在**，而文档站全绿 |

所以 `scripts/smoke-pages.mjs` 就查这三类 + 页面正文非空，
**不重复**构建期已经查过的东西。挂在 `pages.yml` 的 `deploy` 之后。

> 第三条刻意**不**断言「地址等于站点 base」——
> 将来换自定义域名或换仓库时这条不该跟着改。
> 它要守的是「登出去的地址是活的」，不是「是某个字面值」。
...
| 探针 | 结果 |
|---|---|
| 把 `out/_next` 藏起来 | ✓ 报「返回 404 —— .nojekyll 是不是丢了？」 |
| 删掉 `out/r/sidebar.json` | ✓ 报「索引里有，取不到」 |
| 把安装页改成登 `liquid-glass-ui.dev` | ❌ **脚本自己炸了** |

第三个探针没有报错，而是抛了个栈就没了 —— `get()` 重试耗尽后写的是
`throw last`，而域名不存在时 `fetch` 抛的是 Error。

退出码确实非零，CI 也确实会红，但：**没有任何一条说明是哪儿坏了，
而且后面的检查一条都没跑**。更讽刺的是「域名不存在」恰恰是这个脚本
最该说清楚的一种失败 —— 安装页此前登的就是那个地址。

改成连不上时返回一个 response 形状的东西（`status: 0` + `networkError`），
让调用方走正常的 `fail()` 路径。再跑探针：

```
✗ 安装页登的 registry 地址取不到：https://liquid-glass-ui.dev/r/button.json 返回 连不上（fetch failed）
✗ 线上冒烟检查未通过：1 处
退出码=1
```

> 这一条值得单独记：**探针的价值不只是「确认规则会红」，
> 还包括「确认它红得有用」。** 前两个探针都过了，
> 如果就此收工，这个 bug 会一直躺到某天真的域名挂掉 ——
> 而那天正是最需要它说人话的时候。

### 三、验证

对线上 <https://createagle.github.io/liquid-glass-shadcn> 与本地带前缀的静态
...
## 0.85 `@createagle/glass-core@0.1.0` 发布 —— 以及我把「发布失败」判错了（2026-09-05）

从 Phase 5 挂到现在的第一件事，做完了。

| 项 | 值 |
|---|---|
| 包 | `@createagle/glass-core@0.1.0` |
| 协议 | MIT |
| 落库 | 2026-09-05T07:52:56Z |
| 内容 | 80 个文件 · 317.2 kB unpacked |
| shasum | `6ba942a3…` —— 与本机 `npm pack` 逐位相同 |

最后一项值得单独确认一次：**registry 上那个 tarball 与仓库当前状态是同一份东西**，
不是某次中途构建的产物。

### 一、🔴 我判错了一次，而且是「三个证据都指向同一个错误结论」

用户说发好了之后，我查了三处：`npm view` 404、REST 404、scope 搜索 0 命中，
于是断定没发出去，还顺着 npm 的日志推出一套解释
（「PUT 401 → 转去 web auth → 没有重发 → exit 0 骗人」），
并让用户带 `--otp` 重跑一次。

**结论是错的。** 时间线摆出来就清楚了：

```
07:52:25  第二次 npm publish 启动
07:52:56  ← 包真正落库（浏览器里那次 2FA 确认，服务端把它收下了）
07:53:34  ← 我查 npm view，404
```

差 **38 秒**。npm 的读端（registry CDN）对**全新包**有分钟级的同步延迟，
而我那「三个独立证据」全部走同一条读路径 —— **它们根本不独立**。

后来核对 npm 日志：那条 `--otp` 命令**用户根本没跑过**
（`_logs` 里只有 07:51 与 07:52 两次 publish）。
包是第二次那一下发出去的，我让人多跑的那一步是多余的。

> 教训写死在这儿：**新包发布后的 404，在几分钟内不构成任何证据。**
> 判「没发出去」得看写端（publish 的退出状态、日志里的 PUT 结果），
> 或者等足够久再查读端。换多个读接口查同一个 CDN，不叫交叉验证。

### 二、但有两条附带发现是真的，留着

1. **账号是 2FA `auth-and-writes` 模式** —— 每次发布都要 OTP。
   这解释了日志里那个 `PUT 401` 与随后的浏览器确认流程。
2. **npm 在这个路径上的输出确实会骗人**：`PUT 401` 之后开浏览器、
   拿到 `done 200`，然后 `verbose exit 0` / `info ok`，
   **终端里看不到任何一行说「刚才那次 PUT 被拒了」**。
   发布成没成，不能看退出码 —— 要么看落库时间，要么隔一会儿查 registry。

### 三、拆掉本地 shim

`scripts/npm-registry-shim.mjs` 删除，`registry-smoke.yml` 里那段
「打包 + 起 shim + 写 `.npmrc` 指向 localhost:4181」整段拿掉。
冒烟测试从此在干净的 Next 工程里走**真实的 npm 安装路径**。

> 留着的那个 `@glass` 是 **shadcn 的 registry 命名空间**，不是 npm scope ——
> 两者在 workflow 里相邻出现过，注释已经就地写清楚了。

### 四、把「能取，装不上」那批话撤掉

安装页两处、根 README 状态块、`pages.yml` 头注释。这一页的四步现在是端到端通的。

顺手修掉一处**会把 `**` 原样显示给读者**的地方：安装页里
「本机 typecheck 查的是改写`**前**`的源码」写在普通 `<p>` 里，
Markdown 语法在 JSX 里不会被解析，线上就是两对星号。改成 `<strong>`。

### 五、还剩什么

- 🟡 **视觉快照只在本机跑** —— Windows 与 Linux 的 blur 渲染已实测不一致（§0.5），
  要在 CI 上开视觉回归，得先在 Linux 环境录一次基线。
- 🔴 **PROJECT_SPEC §4 / §5 仍写着 `@glass/core`** —— 包名已改，规格没改。
...
## 0.87 Tabs 指示器可拖动 —— 指示器从 trigger 里搬到 List（2026-09-06）

用户直接提的需求：指示器要能拖，拖动与点击都要有强烈的液态玻璃效果。

### 一、旧结构下拖动根本无从谈起

原来的指示器挂在**选中那个 trigger 里**（`absolute inset-0`），
靠 motion 的 `layoutId` 在两个 trigger 之间做共享布局动画。
这套写法下「拖到一半选中态变了」意味着指示器要从一个 trigger 卸载、
在另一个上挂载 —— layout 动画和拖动位移是两个互相打架的位移源。

所以先改结构：**指示器变成 List 里唯一的一个**，用量出来的几何定位。
拖动只是改一个 motion value，选中切换只是改目标值，两件事在同一条数轴上。

顺带清掉的东西：

- `layoutId` 整个没了 —— 连带那个「每个实例必须自带 `useId`，否则同页面
  多组 Tabs 的指示器会塌到同一个坐标」的坑（§0.63）**不可能再发生**；
- 每个 trigger 各自一套 `MutationObserver` + `ResizeObserver` 收拢成 List 里的一套；
- trigger 里那份「我是不是选中」的 React state 也删了 ——
  颜色与高亮层改走 `data-[state=active]` / `group-data-[state=active]` 的 CSS 变体。

**静止外观逐像素不变**：286 张视觉快照一张没动，两个平台都不用重录。

### 二、🔴 `.click()` 什么都不会发生

拖动中要实时改选中值，而 Radix 不把 value 的 setter 暴露出来，
只能从 DOM 这侧发事件。第一版写的是 `segs[target].el.click()` ——
**指示器跟着手指走、宽度也在变，唯独选中态纹丝不动，而且不报任何错。**

翻 Radix 源码才看明白：`TabsTrigger` 只挂了 `onMouseDown` / `onKeyDown` / `onFocus`，
**压根没有 `onClick`**。改成派发 `mousedown`（还要带 `button: 0`，
它里面判了 `event.button === 0 && !ctrlKey`）才对，
再补一次 `focus({ preventScroll: true })` 让 roving tabindex 跟过去。

> 这一条值得记：**「点一下」在 DOM 上不是一个动作，是一串。**
> 组件库监听哪一环是它的自由，从外面合成事件就必须按它监听的那一环来。

### 三、🔴 探针把一条「绿的但没测到东西」的测试抓了出来

「挖洞必须跟着指示器走」是这次最关键的一条 —— 洞停在原处的话，
指示器一拖出洞就只能看到被底座模糊过的背景，折射当场失效，
玻璃在半路变成一块糊。写完先按规矩上探针：把洞对位置的订阅摘掉。

**探针没红。**

原因：第一版测试是**拖过两段**的，跨段时宽度也在动，
而宽度那条订阅顺手把洞刷新了 —— 于是这条测试是靠**另一条链路**通过的，
根本没验到自己声称的东西。

改成**同一段内小幅拖动**（宽度不变、选中态不变，位置是唯一变量）之后，
探针立刻红，还原后绿。

> 与 §0.83 那次同源：探针的价值不只是「确认规则会红」，
> 还包括**确认它是因为你想验的那件事才红**。

另有两条断言是我自己算错、看着却像组件的 bug：

- 洞的 `clip-path` 起点写的是 `M (x + r) y`，读出来天然带一个圆角半径的偏移，
  我拿它跟 `x` 比，差了 27px；
- 指示器按住时有 1.03 的放大，`boundingBox()` 比布局位置左 1px ——
  挖洞用的是布局位置，所以该读 transform 矩阵，不是视觉盒子。

### 四、强度阶梯（全部 [推定]）

手势在两份设计资源里都没有依据，所以这一族数值一个都不敢标 [实测]：

| 状态 | 折射 | 色散 | 其他 |
|---|---|---|---|
| 静止 | 2 | 2 | — |
| 按住 | 3（`pressed` 内部 +1 档） | 2 | 提亮 1.12 / 饱和 1.4、投影加深、放大 1.03 |
| 拖动 | 3 | **3** | 再加速度驱动的横向拉伸（上限 1.06）与配套纵向收缩 |

色散推到 3 是刻意的：可见的彩色分离是 Layer I 的签名特征
（PROJECT_SPEC §2 要求指示器「必须有可见色散」），静止时不该这么强 ——
对应 Apple 那句 "the knob transforms into Liquid Glass during interaction"。

其余约束：只有当前选中段能起拖（从别的段起手要同时处理「指示器正飞过来」
和「手指在拖」两个位移源，行为无法预测，iOS 上也只有选中段可拖）；
3px 以内的抖动不算拖动；禁用段照样占位置但不能成为落点；
`touch-action: pan-y` 让纵向滚动仍归页面；
`prefers-reduced-motion` 下拉伸与放大整条链路不参与。

### 五、顺手：docs 测试不再复用现有服务

`playwright.docs.config.ts` 的 `reuseExistingServer` 这次又骗了我一回 ——
上一轮 `test:docs` 留下的 `next start` 还活着，伺服的是**上一次的构建产物**，
47 条里只跑起 32 条，其余超时。看起来像测试挂了，实际是在测旧代码。

2026-09-03 那次是 `next dev` 占着 4200，形态一模一样：
**「有个服务在那儿」不等于「它是对的那个」。** 踩两次就别再靠注释提醒了，
改成每次自己构建自己起。代价是本地多等一分钟左右。
...
- **没有真机验证过手感。** 拉伸上限、阈值、放大量都是拍出来的 [推定]，
  没有 iOS 参考视频可比。
- **视觉快照没覆盖拖动中的样子。** 拖动是个瞬时态，
  截图要么钉死在某一帧（脆），要么得把弹簧停掉（就不是它了）。
  现在靠行为测试钉住「位置 / 洞 / 选中态 / data-dragging」这些确定性事实。
- `sheet` 甩动那条 flake 仍然没修（§0.84 七）。
...
**P2 第四批已交付**（§0.78）：`Calendar` / `DatePicker` / `Combobox`。
上一批那句「先量 `507:24680`」是对的 —— 那一页同时给出了 Calendar 与
DatePicker 两个形态，几何全部共用。
**而且给出了本库第一次拿到的一条 Apple 自证**：同一个日历，
`Style=Inline`（嵌在内容里）是纯白不透明、`Style=Compact`（弹出层）才是玻璃。
PROJECT_SPEC §2「材质属于控件层」此前一直只能靠推理，这次有了直接证据。

> 顺带修正了一处对 `Combobox` 的判断：清单写的「UIPickerView + 搜索」对不上 ——
> UIPickerView 是滚轮，**不能打字**。参考只能是 macOS `NSComboBox`
> （与 Checkbox / Radio 同一种情况），且**只取结构与状态，不取尺度**。

✅ **P2 十六个全齐了**（§0.79）：收尾批交付 `DataTable` 与 `Command`。

这一批的结论有一半是**否定式**的，而那恰恰是「先量再定」第五次的价值：

- `DataTable` **一个新几何常量都没有** —— 需要的外观全都已经在 §11.4 量过、
  并且已经实现在 `<Table>` 里；增量是排序 / 选择 / 分页，那是**交互**不是外观。
  「量过了，发现没有新的可量」要写下来，否则下一个人还得再翻一遍。
- `Command` 在 Apple 资源里**结构上不可能有** —— Spotlight 是系统级的，App 画不出来。
  唯一可量的是搜索框（macOS `480:760`），而那是鼠标语境的 24 高控件，只取结构不取尺度。

🔴 **顺手挖出一个挂了两个月的洞**：`registry-lint` 的注释剥离器在**吞真实代码**，
已有的 `no-as-child` 规则实际覆盖面一直比看上去小。同一批里「跑得过 + 什么都没查」
这个形态出现了**三次**。修好之后新加的规则一上线就抓出三处真 bug，
其中一处是上一批留下的、**没有任何测试发现的**。详见 §0.79 第三节。
...
4. ✅ **`@createagle/glass-core@0.1.0` 已发布到 npm（MIT）** —— 见 §0.85。
   本地 shim 已拆掉，冒烟测试走真实 npm 安装路径；安装页四步端到端通。
