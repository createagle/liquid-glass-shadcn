...
全绿之后，一个**只改文档**的提交上审计又红了一次，重跑即通过 ——
说明它在 CI 上是**非确定性**的。

失败时：`dark / 档位 0.34 / Tier b / mid`，背后像素 `rgb(111,111,112)`，
反推有效 alpha 约 **0.25**，而设定值是 **0.696**。也就是说材质没按设定渲染。

提出过一个假说：截到了 `background-color 240ms` 过渡的中间态。
据此关掉了两个夹具的 transition/animation，并加了静止态守卫
（连续两帧颜色一致才继续截图）。

⚠️ **但这个假说在本机被证伪了。** 故意注入 10 秒过渡后守卫没触发、
数字也没变 —— 夹具是在内联 script 里设变量的，发生在首次绘制之前，
元素根本没有「旧值」可过渡。

**结论：加固已做（无害），根因未明。** 本机连跑三次完全一致，复现不了。
下次 CI 再红时看守卫是否报错：
- 守卫报错 → 确实是稳定态问题，方向对
- 守卫没报 → 问题不在稳定态，需要换方向（怀疑对象：headless 软件光栅下
  `backdrop-filter` 的合成时机，或 Playwright 截图与合成器不同步）

在查清之前，**不能把这个审计当作可靠闸门** —— 它可能随机放过真实回归。
