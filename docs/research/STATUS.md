...
### P1 只剩 Checkbox / Radio Group —— 但它们卡在一个**该由人拍板**的问题上

清单标着「无 iOS 对应（iOS 用 checkmark 行）；macOS NSButton checkbox」。
也就是说：**在 iOS 上，Apple 根本不用复选框** —— 表单里的多选是
「一行文字 + 右侧对勾」，那个形态本库已经有了（`Card` + `CardRow`）。

而本项目手上只有 **iOS 27** 的设计资源，没有 macOS 的。硬做出来的结果是：
每一个数字都是 `[推定]`，且**没有任何东西能证伪它**。
前三批建立起来的标准（每个数字标清来源、没有依据就说没有）与这种组件是冲突的。

三条路，需要你选：

1. **先找 macOS 参考**（Apple Design Resources 有 macOS 的 Figma 文件），
   拿到之后再做 —— 与前三批同一个标准。
2. **明确接受「这两个就是没有依据的」再做**，像 Skeleton 那样在组件头部写死
   「Apple 没有这个东西，几何全部推定」。
3. **不做**，转而按 iOS 的实际做法提供 `CardRow` 的 checkmark 用法文档 ——
   那才是 Apple 在 iOS 上的答案。

在这个问题定下来之前，可以先做 **P2 的前几个**（Accordion / Collapsible /
Scroll Area / Table），它们是内容层，且 Scroll Area 正好能复用
Hero 那批做的滚动边缘效果。
