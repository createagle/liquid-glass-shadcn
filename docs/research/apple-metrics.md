...
>
> ✅ **颜色告警已结案（2026-09-05，见 §9.1）：** `rgb(0 136 255)` 当时被怀疑是
> Display P3 → sRGB 的转换差异，所以写明「不得据以修改 token」。
> 后来累计到**四份互相独立**的资源都量到逐位相同的值，色彩管理的解释被排除 ——
> `--lg-blue` 已改成 `#0088ff`，`#007AFF` 是上一代的值。
...
### 9.1 `--lg-blue` 结案 —— 强调蓝是 `#0088ff`，不是 `#007AFF`（2026-09-05）

上面那条「Apple 保留随版本调整的权利」的警告，**在蓝色上真的应验了**。
...
**四份互相独立的资源逐位相同** —— 不可能是某一份文件的色彩管理误差。
§7.4 当初怀疑的 Display P3 → sRGB 转换差异，可以排除了。

- `--lg-ios-blue-light`：`#007aff` → **`#0088ff`** `[实测]`
- `--lg-ios-blue-dark`：`#0a84ff` → **`#0d9eff`** `[实测]`

> ⚠️ **暗色只有一份来源，而且是 macOS。** §10.3 里暗色选中底写成
> `#0091ff` + `#0d0d0d` LINEAR_DODGE，等效实色 `#0d9eff`。
> 取的是**等效值**而不是那层基色 —— 与亮色用的是同一条规则
> （「Idle 态渲染出来的强调填充」；亮色 Idle 没有混合层，等效即基色）。
> 本库不会在填充上再叠一层 dodge，取基色会比 Apple 暗 13/13。
>
> iOS 的资源里一个暗色样例都没有（`findAll(/dark/i)` 返回空）。
> **亮四份、暗一份，把握程度不一样，别把两者当同等确定。**

#### 代价：白字更不占便宜了

新的强调蓝更亮，**白字压上去的对比度反而更低**，AA 修正因此要拉得更狠：

| | 原色白字 | 解出的填充 | 修正后 | 每通道位移 |
|---|---|---|---|---|
| 亮 `#007aff` → 现 `#0088ff` | 4.02 → **3.52** | `#0071eb` → **`#0075da`** | 4.60 → 4.61 | G/B −9/−20 → **−19/−37** |
| 暗 `#0a84ff` → 现 `#0d9eff` | 3.65 → **2.86** | `#0973df` → **`#0a79c4`** | 4.64 → 4.62 | G/B −17/−32 → **−37/−59** |

暗色那一档是**全库偏离 Apple 最远的一个色值**。
换成黑字能一步解决（`#0d9eff` 黑字 7.35:1），但 iOS 的蓝色 prominent 按钮就是白字 ——
极性属于设计语言，不由算法挑，见 `deriveProminentFill()`。

#### 一条反直觉的观测：视觉快照**一张都没动**

改完跑 286 张视觉快照，**全绿**。原以为会全军覆没。

原因是 Playwright 的默认 `threshold: 0.2` 是 pixelmatch 的**逐像素 YIQ 距离**阈值
（0.2 × 35215 ≈ 7043），而这次所有色变都在两个数量级之下：

| 变化 | pixelmatch delta | 阈值 |
|---|---|---|
| `--lg-blue` 暗（最大的一处） | **175** | 7043 |
| `--lg-blue` 亮 | 49 | 7043 |
| `accent-fill` 暗 | 57 | 7043 |
| `accent-fill` 亮 | 22 | 7043 |
| `on-glass-blue` 亮/暗 | 6 / 4 | 7043 |

> **结论要写清楚：那 286 张快照守的是几何与版式，不是颜色。**
> 这个阈值是为抗锯齿噪声调的，本来就不该指望它管色准。
>
> **不打算调紧它** —— 已实测 Windows 与 Linux 的 blur 渲染本来就不一致（STATUS §0.5），
> 调紧会让整套快照在平台间全红。颜色的事归**行为测试**用 `getComputedStyle` 钉，
> 这本来也是本库既有的做法。本次因此补了两条**字面色值**断言
> （日历「今天」、Button 实心填充），此前它们都只断言「走了 token」——
> 那种写法在 token 本身被改错时照样绿。
...
## 15. P2 收尾批 —— Data Table 没有新几何；Command 只有搜索框有依据

第五次「先量再定」。这一次的结论有一半是**「不用量」**，如实记下来。

### 15.1 Data Table：一个新数字都没有 `[实测]` 已在 §11.4

回去把 macOS `207:14499 Lists and Tables` 又翻了一遍，
Data Table 需要的东西**全都已经在 §11.4 里量过并且已经实现**：

| 部件 | 节点 | 状态 |
|---|---|---|
| 列表头 | `121:12610 _Column Headers`（4 变体） | ✅ §11.4，`table.tsx` 已实现 |
| 整条表头 | `4356:13469 Column Header` 600×28 | ✅ §11.4 |
| 排序指示器 | `4356:13719 Sort Order Indicator`（升/降） | ✅ §11.4，`SortIndicator` 已实现 |
| 数据行 / 交替行 / 两档选中 | `4356:11854 Table Rows`（20 变体） | ✅ §11.4 |

复核了一遍排序指示器：`􀆇` / `􀆈`，**SF Pro Medium 9**，框 13×19，`#000000 @ 0.50`
—— 与 §11.4 记的逐位相同。列表头非首列带一条 `#000000 @ 0.1` 的分隔描边，
首列左内边距 **50**（给大纲视图的展开三角让位）。

> **所以 `DataTable` 的增量是行为，不是材质，也不是几何。**
> 排序状态机、行选择、分页 —— 这三件 Apple 的设计资源里本来就不会有，
> 它们是**交互**，不是外观。这一节存在的意义就是把这件事写清楚：
> 不是「懒得量」，是**量过了，发现没有新的可量**。
>
> ⚠️ 首列左内边距 50 那一条**没有采用**：它是给 `Outline View` 的展开三角留的，
> 而本库的 `DataTable` 不做大纲视图（`Table` 的 `level` 已经能表达层级）。
> 记在这里是为了说明「看见了、并且选择不用」。

### 15.2 Command：面板与列表**没有依据**，只有搜索框有 `[推定]` + `[实测]`

⚠️ **Spotlight 是系统级的，App 画不出来，所以两份设计资源里都不会有它。**
这不是「没找到」，是**结构上不可能有** —— 与 Toast 的「系统通知横幅」同一类。
逐页确认过：iOS 39 页、macOS 39 页，都没有命令面板。

**唯一有实测依据的一半是搜索框**（macOS `480:760 Search Field`，6 个状态）：

| 项 | 值 |
|---|---|
| 控件高 | **24**，圆角**胶囊**（r=100），`cornerSmoothing 0.6` |
| 内边距 | 左右 **8**，元素间距 **2** |
| 放大镜 | `􀊫`，SF Pro **Medium 13**，框 16×15，`#000000 @ 0.85` |
| 文本 | SF Pro **Medium 13** / 行高 16，`#000000 @ 0.85` |
| 清除按钮 | `􀁡`，Medium 13，框 16×15，`#000000 @ **0.5**` |
| 文本光标 | 2 × 18，r=100，`#0088ff` |
| 底 | `#ffffff`；描边靠一层 `MULTIPLY` 白 + `rgba(0,0,0,0.08)` 硬边投影 |

> ⚠️ **尺度照旧不取**（同 §13.4 / §14.6）：24 高 / 13 号字是鼠标语境。
> 本库取**结构与比例**（放大镜在前、清除在后、胶囊、光标蓝），
> 尺寸走本库既有的 Input / DropdownMenu。
>
> 顺带一条独立印证：这里的光标又是 **`rgb(0,136,255)`** ——
> 这是 `#0088ff` 的**第四份**独立来源（前三份见 §10 / §14.2 / §14.5）。
> ✅ 这第四份来源凑齐之后，`--lg-blue` 已经改了，见 §9.1 与 STATUS §0.81。

**没有依据的部分**（面板宽 / 圆角 / 分组标题 / 空态 / 快捷键提示）
一律借自有实测的邻居，并在组件里逐条写明借自哪里 ——
...
| §6 iOS 系统色表 | blue `#007AFF` / `#0A84FF`，整表标注为「已核实，直接用」 | **`#0088ff` / `#0d9eff`**（亮色四份独立实测，暗色一份） | ❗**待修订**，见 §9.1。其余八色未被推翻，但整表那句「已核实」现在已知**至少有一行是错的** |
...
| ~~4~~ | ~~`rgb(0 136 255)` vs `#007AFF` 的差异是色彩管理还是真实改值？~~ | ✅ **已解决**（§9.1）：四份独立来源逐位相同，是**真实改值**。`--lg-blue` 已改成 `#0088ff` / `#0d9eff` |
