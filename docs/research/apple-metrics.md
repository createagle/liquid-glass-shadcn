...
## 14. iOS 27 Date and Time Pickers / macOS 27 Combo Box

P2 第四批（Calendar / Date Picker / Combobox）动手前的测量。
连续第四次「先量再定」，这次**没有改批次** —— 三个都查到了依据。

数据来源：iOS 文件 `ojEQo0rKaQ5ioARo0CO0pf` 的 `507:24680 Date and Time Pickers`；
macOS 文件 `dRTOe4ObAK8UGqW9CBoJPM` 的 `207:14473 Combo Boxes`。

### 14.1 ❗ 两种形态，材质不同 —— 本节最重要的一条 `[实测]`

节点 `5442:1885`（Date and time - Pickers，两个变体）。

| | 圆角 | 左右内边距 | 材质 |
|---|---|---|---|
| `Style=Inline`（嵌在内容里） | 无 | **16** | **纯白不透明**（`#ffffff` a=1） |
| `Style=Compact`（弹出层） | **13** | **12** | **玻璃** |

Compact 那一档挂着共享的 `Material` 组件（`533:16442`），两层：

```
Fill + Shadow   r=13   #ffffff @ 0.7 LIGHTEN + #bfbfbf @ 0.1 DARKEN
                       DROP_SHADOW r=48（外加三条 r=0 的硬边）
Glass Effect    r=13   #ffffff MULTIPLY，fx=GLASS r=16，两条 INNER_SHADOW r=10
```

> **这正好是 PROJECT_SPEC §2「内容型组件不堆玻璃」的一个 Apple 自证。**
> 同一个日历，**嵌在内容里就是纯白的，浮成弹出层才是玻璃**。
> 本库据此把 `Calendar` 做成内容层、`DatePicker` 的弹层做成 Layer B ——
> 不是按规范推的，是资源里就这么分的。

### 14.2 日期格 `[实测]`

节点 `50:63907`（_Day，5 个变体）。格子 **38 × 38，正圆**（r=100）。

| 状态 | 底 | 文字 |
|---|---|---|
| Default | 无 | `#000000`，SF Pro **Regular 20** / 行高 24 / 字距 **−0.45** |
| Current（今天） | 正圆 `#0088ff @ 0.12` | `#0088ff`，Regular 20 |
| Selected | 实心 `#000000` | 白，**Semibold** 20 |
| Current and Selected | 实心 `#0088ff` | 白，Semibold 20 |
| Null（非本月） | 空 | —— 整格什么都不画 |

> 注意 **Selected 是黑底白字，不是蓝底**；只有「今天且被选中」才是蓝底。
> 这与直觉相反（多数 Web 日历把选中画成主题色），但资源里就是这样。

### 14.3 排布 `[实测]`，外加一处 kit 自身的不一致

节点 `50:63951`（_Week）与 `5442:1477`（Calendar）。

- **横向**：7 格，节距 **50**（格 38 → 间隙 **12**），一周共 **338**。
- **纵向**：`Calendar` 的 auto-layout `itemSpacing = 7`，上内边距 3。

⚠️ **样例里对不上**：`Light examples` 那份组合稿里，第 1 周行高 38、
第 2–6 周被手工拉成 **44**（格子在行内再下移 3），于是实际节距变成 51，
而组件自身是 38 + 7 = **45**。

> 取**组件值**。`itemSpacing` 是一条显式的 auto-layout 属性，
> 而 44 是在组合稿里手拉出来的 —— 前者是规格，后者是排版。
> （与 §12.1 那条「7 页时点间距画成 10」是同一类，处理方式也一致。）

### 14.4 面板的其余部件 `[实测]`

| 部件 | 值 |
|---|---|
| 月份行 | 高 **40**，内边距 上 13 / 右 6 / 下 3 / 左 8 |
| 月 + 年 | SF Pro **Semibold 17** / 行高 22 / 字距 −0.43；后跟展开箭头 `#0088ff` 13 Bold |
| 上/下月箭头 | 15 × 24，字号 20，两者间距 **28** |
| 星期表头 | 高 **20**；缩写 38 宽、节距同 50；SF Pro **Semibold 13** / 行高 18 |
| 星期缩写颜色 | `#3c3c43 @ 0.30` —— **又一次正好落在 `--lg-label-tertiary` 上** |
| 主体 | 上下内边距 8，与表头间距 4 |
| Time 行 | 高 **52** |
| Inline 变体的分隔线 | `#000000 @ 0.12`，只在 Date 块下缘 |

### 14.5 折叠字段（DatePicker 的触发器）`[实测]`

节点 `30:53803`（Date and time - Collapsed，Default / Selected）。

| 项 | 值 |
|---|---|
| 整体 | 204 × **34**，两枚之间间距 **6**，外框 r=6 |
| 日期枚 | 112 × 34，**r=100（胶囊）**，`cornerSmoothing 0.6` |
| 时间枚 | 86 × 34，同上 |
| 填充 | `#767680 @ 0.12` —— **= `--lg-fill-tertiary`**（R/G 各差 2，同 §13.3 的漂移） |
| 内边距 | 上下 6 / 左右 **11** |
| 文字 | 17 / 行高 22 / 字距 −0.43 |
| 选中态 | **只有文字变色**（转 `#0088ff`），底色不变 |

> 这是 `#787880` 系填充第 **四** 次命中既有 token。本批同样**没有新增颜色 token**。

### 14.6 macOS Combo Box `[实测]`

节点 `121:11951`（Combo Box，4 个状态）。iOS 没有这个控件 ——
与 Checkbox / Radio 一样，只能取 macOS（见修订三的先例）。

| 部件 | 值 |
|---|---|
| 控件高 | **24**（macOS medium 档），内边距 上下 2 / 右 2 / 左 0 |
| 文本域 | 圆角 **6**；左内边距 8、**右 28**（给按钮让位） |
| 下拉按钮 | **24 × 20**，圆角 **4.5**，`cornerSmoothing 0.6` |
| 按钮底 | idle `#000000 @ 0.08` · 按下 **0.16** · 禁用 **0.04** |
| 按钮箭头 | `􀆈` 11 Bold，`#000000 @ 0.85` |
| 文本 | SF Pro **Medium 13** / 行高 16，`#000000 @ 0.85` |
| 焦点环 | `#0088ff` 内外双描边 @ 0.5 —— **与 §10.3 的焦点环是同一个** |
| 文本光标 | 2 × 18，r=100，`#0088ff` |

⚠️ **尺度不采用**，理由与 §13.4 的 macOS 侧栏行高一致：
24 高 / 13 号字是鼠标语境的产物，本库基准是 iOS（控件 44、正文 17）。
**结构与状态取自 macOS，尺寸走本库既有的 Input**，组件里逐条标明哪个是哪个。

### 14.7 Command：两份资源都没有，而且**有明确的理由** `[推定]`

`Command`（命令面板）对应的是 Spotlight —— **那是系统级的，App 画不出来**，
所以设计资源里当然不会有它的样例。这与 §12 里 Toast「系统通知横幅」是同一类。

本批**没有做 Command**：它和 `Data Table` 一起留到 P2 最后一批。
记在这里是为了说明「查过了、知道没有」，而不是漏掉。
